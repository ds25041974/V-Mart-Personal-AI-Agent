...
try:
    ...
    gemini_engine = (
        GeminiRetailInsights(api_key=gemini_api_key) if gemini_api_key else None
    )
...
def chat_with_ai():
    ...
    try:
        ...
        if not gemini_engine:
            return jsonify(
                {"success": False, "error": "Gemini AI engine not initialized"}
            ), 500
        ...
        if any(greeting in query.lower() for greeting in greeting_words):
            ...
            return jsonify({"success": True, "response": response, "type": "greeting"})

        # General AI insights
        response = gemini_engine.get_insights(
            query=query, context_data=context_data, analytics_type="general"
        )
...
def analyze_salesperson_performance():
    ...
    try:
        ...
        result = sales_analyzer.analyze_salesperson_performance(
...
def upload_and_analyze_file():
    ...
    try:
        ...
        result = data_integration_manager.process_uploaded_file(
            file_path=file_path, data_context=data_context
        )
...
def analyze_google_sheet():
    ...
    try:
        ...
        if not spreadsheet_id:
            return jsonify(
                {"success": False, "error": "spreadsheet_id is required"}
            ), 400

        # Connect to Google Sheets
        if credentials_path:
            connection_result = data_integration_manager.connect_google_sheets(
                credentials_path
            )
...
def query_and_analyze_database():
    ...
    try:
        ...
        # Connect to database
        connection_result = data_integration_manager.connect_database(
    ...
    try:
        ...
        if analytics_type == "sales":
            ...
            if "hourly" in query.lower():
                result = sales_analyzer.analyze_hourly_sales(
                    context_data.get("hourly_data", []), context_data.get("date", "")
                )
            else:
            ...
            if "image" in context_data:
                ...
                result = fashion_analyzer.analyze_global_trends(
                    context_data.get("season", "Summer"), context_data.get("year", 2025)
                )
            else:
